...
Worker. Значение по умолчанию `5` означает первую попытку и пять повторов через
1/3/5/7/9 секунд; `0` оставляет только первую попытку. Постоянная ошибка
...
заблокированный Worker требует перезапуска своего runtime/процесса.
